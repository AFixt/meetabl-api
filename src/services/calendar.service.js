...
// Lazy load heavy dependencies to avoid Lambda initialization issues
let google, Client;
...
const loadMicrosoftGraphClient = () => {
  if (!Client) {
    const { Client: C } = require('@microsoft/microsoft-graph-client');
    Client = C;
  }
  return Client;
};
...
const getGoogleAuthClient = async (userId) => {
  try {
    ...
    const googleAPI = loadGoogleAPIs();
...
const getMicrosoftGraphClient = async (userId) => {
  try {
    ...
    const ClientClass = loadMicrosoftGraphClient();
    const client = ClientClass.init({
      authProvider: (done) => {
...
async function getGoogleUserEmail(userId, accessToken) {
  try {
    ...
    const googleAPI = loadGoogleAPIs();
    const oauth2 = googleAPI.oauth2({ version: 'v2', auth: oauth2Client });
