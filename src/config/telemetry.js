...
let Resource, SemanticResourceAttributes, NodeSDK, PrometheusExporter, opentelemetry;

// Try to load OpenTelemetry modules
try {
  const autoInstrumentation = require('@opentelemetry/auto-instrumentations-node');
  NodeSDK = autoInstrumentation.NodeSDK;
  ...
  logger.warn('OpenTelemetry modules not properly installed, telemetry will be disabled', { error: error.message });
}
...
class TelemetryManager {
  ...
  initialize() {
    ...
    if (!Resource || !NodeSDK) {
      logger.warn('OpenTelemetry modules not available, skipping initialization');
      return;
    }
    ...
    try {
      ...
      // Don't throw, just log the error and continue without telemetry
    }
  }
